...
void initialize_agent(struct head *tmp_agent) {
    tmp_agent->crencas = tmp_agent->objetivos = tmp_agent->planos = NULL;
}

void set_agent_name(struct head *tmp_agent, const char *name) {
    strncpy(tmp_agent->name_agent, name, sizeof(tmp_agent->name_agent) - 1);
    tmp_agent->name_agent[sizeof(tmp_agent->name_agent) - 1] = '\0';
}

void print_agent_details(struct head *tmp_agent) {
...
struct node *allocate_goal_node(const char *name) {
...
void add_new_goal(struct head *a, struct node *new) {
    new->next = a->objetivos;
    a->objetivos = new;
}

void print_goal_details(struct node *new) {
    printf("OBJETIVO <%s>\n", new->name);
}
...
struct node *allocate_plan_node(const char *name, const char *eg, struct expr *exp, struct list *list) {
    struct node *new = (struct node *)malloc(sizeof(struct node));
    if (!new) {
        yyerror("[!] Memory allocation failed.\n");
        exit(EXIT_FAILURE);
...
void add_new_plan(struct head *a, struct node *new) {
    new->next = a->planos;
    a->planos = new;
}
...
void set_new_plan(struct head *a, const char *name, const char *eg, struct expr *exp, struct list *list) {
...
struct expr *allocate_expression() {
